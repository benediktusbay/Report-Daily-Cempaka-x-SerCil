...
def manage_program_ppg_paa():
    ...
    if action == 'deactivate':
        if not participant:
            flash('Peserta Program PPG tidak ditemukan.', 'danger')
        else:
    ...
    if action not in ('create', 'update') or (action == 'update' and not participant):
        flash('Aksi Program PPG tidak valid.', 'danger')
        return redirect(return_url)
    ...
    try:
        ...
        flash('Target atau tanggal Program PPG tidak valid.', 'danger')
        return redirect(return_url)
    if (not re.fullmatch(r'\d{1,20}', bp)
        ...
        flash('Lengkapi data peserta Program PPG dengan nilai yang valid.', 'danger')
        return redirect(return_url)
    ...
    for other in ProgramPpgPaaParticipant.query.all():
        ...
        if all_codes & ({other.bp_code} | set((other.billing_bp_aliases or '').split(','))):
            flash('BP sudah terdaftar di master Program PPG.', 'danger')
            return redirect(return_url)
    ...
    db.session.commit()
    flash('Master Program PPG berhasil disimpan.', 'success')
    return redirect(return_url)
